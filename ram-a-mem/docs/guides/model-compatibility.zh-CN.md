...
1. 确认请求字段被端点接受，且密钥不会出现在日志；
2. 分别测试普通最终 content、同时存在 reasoning/content、reasoning-only；
3. 测试所选 Structured Output 模式；
4. 使用实际 Extract/Ground 输出预算跑一次完整 `memory_ingest`；
5. 检查 accepted、rejected、quarantined、memory IDs 与结构化日志；
6. 执行 `memory_search`，重启服务后再次检索，确认持久化。

## 当前边界

- 不清理混在最终 content 正文里的思维链；
- 不支持任意 Provider 私有协议或自动 Backend 切换；
- 不自动识别模型上下文窗口；
- 不自动降级 Structured Output；
- 不改变 `fail_fast`：Extract/Ground 最终失败后，仍按现有配置决定终止或跳过窗口。
