...
配置的全部字段，并说明两层默认值、有效范围、生效条件和测试要求。完整配置示例见
[`plugins/mcp/ram-a-mem.json`](../../plugins/mcp/ram-a-mem.json)。

### 默认值的两层机制

字段表中有两个含义不同的默认值列，阅读时必须区分：

| 列名 | 含义 |
| --- | --- |
| 未设置兜底值 | JSON 中**省略该字段**时，服务端反序列化自动填充的值。只作用于被省略的字段，与交付配置无关 |
| 交付配置默认值 | 交付配置文件 [`plugins/mcp/ram-a-mem.json`](../../plugins/mcp/ram-a-mem.json) 实际写出的值。该文件被源码测试锁定为全字段显式写满，因此交付环境实际生效的**总是这一列** |

两层不一致的典型字段是 `http.port`：省略时兜底为 `8080`，交付配置写 `18081`。交付环境
安装后请以 `/etc/ram-a/ram-a-mem.json`（或部署使用的配置文件）实际内容为准；两层一致
的字段（如全部 `limits`、`pipeline` 值）省略与显式写出效果相同。

本文约束列中以“生产部署建议”标注的内容，不随交付配置默认值生效，需按环境显式覆盖。
...
| 推荐可配置 | 允许用户覆盖；表中交付配置默认值是已确认的交付配置 | 验证未设置兜底值、交付配置默认值、上下边界和越界值 |
...
- 省略 `http.port` 时的未设置兜底值是 `8080`；RPM/本项目交付示例的交付配置默认值是
  `18081`。两者不能混写为同一种默认值。
...
交付配置默认值中 `auth`、`storage`、`providers`、`case_library`、`graph_memory` 均为需按
环境替换或注入密钥的示例值，见对应章节。
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 约束与生效条件 | 测试要求 |
| --- | --- | --- | --- | --- | --- |
| `tokens` | 部署必配 | 无（必填） | 一项 | 非空数组 | 空数组启动失败；多 Token 可启动 |
| `tokens[].token_env` | 部署必配 | 无（必填） | `RAM_A_XIAOO_TOKEN`（示例，按客户端替换） | 非空、无前后空格；数组内唯一；环境变量必须存在且值非空 | 缺失、空值、重复环境名及重复实际 Token 均失败；配置和日志不得出现密钥值 |
| `tokens[].tenant_id` | 部署必配 | 无（必填） | `tenant-local`（示例，按环境替换） | 非空、无前后空格；参与 `scope_id` | 相同/不同租户的 scope 隔离 |
| `tokens[].user_id` | 部署必配 | 无（必填） | `alice`（示例，按环境替换） | 非空、无前后空格；参与 `scope_id` | 相同/不同用户的 scope 隔离 |
| `tokens[].agent_id` | 部署必配 | 无（必填） | `xiaoo`（示例，按客户端替换） | 非空、无前后空格；`x-agent-id` 提供时必须一致 | Header 缺省、匹配和不匹配 |
| `tokens[].permissions` | 固定值集合 | 无（必填） | 四种权限全开 | 元素只能是 `memory:read`、`memory:write`、`cases:read`、`cases:write`，同一 Token 内不得重复；空数组表示只能认证、不能调用受保护工具；生产部署建议：按最小权限裁剪 | 四种合法值、未知值、重复值和权限不足 |
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 约束与生效条件 |
| --- | --- | --- | --- | --- |
| `memory.enabled` | 推荐可配置 | `true` | `true` | 控制 `memory_ingest`、`memory_search`；图记忆开启时必须为 `true` |
| `case_library.enabled` | 条件配置 | `null` | `true` | `null` 时由顶层 `case_library` 是否存在决定；`true` 时必须提供 `case_library` |
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 约束与生效条件 |
| --- | --- | --- | --- | --- |
| `bind_address` | 推荐可配置 | `127.0.0.1` | `127.0.0.1` | IP 地址；非 loopback 时触发 TLS 和 Host 额外校验；生产部署建议：单机/反向代理部署使用 `127.0.0.1` |
| `port` | 推荐可配置 | `8080` | `18081` | `u16`；应避免 `0` 和已占用端口，端口可绑定性由启动测试验证 |
| `allowed_origins` | 部署可配置 | `[]` | `["http://127.0.0.1:18080"]` | 精确匹配允许的 Origin；不影响非浏览器客户端；生产部署建议：无浏览器跨域需求时设为 `[]` |
| `allowed_hosts` | 部署必配 | `localhost`、`127.0.0.1`、`::1` | `["127.0.0.1:18081"]` | 非空数组，元素非空；外部监听至少包含一个非 loopback Host；写实际访问 Host，包含端口时也应与请求一致 |
| `tls_termination_acknowledged` | 条件配置 | `false` | `false` | 只表示部署者确认，不会让 RAM-A 自己启用 TLS；外部监听且已有 TLS termination 时设 `true` |

测试应覆盖本地默认配置、外部监听未确认 TLS、仅 loopback Host、外部 Host 正确配置四种情况。

## 6. `limits`

以下未设置兜底值与交付配置默认值相同，配置时不能超过固定支持范围。

| 字段 | 交付配置默认值 | 固定有效范围 | 作用 |
| --- | ---: | ---: | --- |
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 固定范围 | 生效条件与结果 |
| --- | --- | --- | --- | --- | --- |
| `fail_fast` | 推荐可配置 | `true` | `true` | `true`/`false` | 只控制 Extract/Ground 窗口错误；`true` 终止本次摄入，`false` 跳过失败窗口并继续 |
| `max_memory_chars` | 推荐可配置 | `500` | `500` | 1..=32000 Unicode 字符 | 抽取记忆超限进入 quarantine，不截断 |
| `max_candidate_tokens` | 推荐可配置 | `320` | `320` | 大于 0 | Window 阶段单个候选窗口预算，使用本地启发式 token 估算 |
| `max_window_tokens` | 推荐可配置 | `640` | `640` | 不小于 `max_candidate_tokens` | Window 阶段候选加上下文总预算，使用本地启发式 token 估算 |
| `extractor_max_output_tokens` | 推荐可配置 | `1600` | `1600` | 大于 0 | Extract Chat 请求的最大输出 token |
| `verifier_max_output_tokens` | 推荐可配置 | `1000` | `1000` | 大于 0 | Ground Chat 请求的最大输出 token |
| `extractor_context_window_tokens` | 条件配置 | `null` | `null` | `null` 或大于 `extractor_max_output_tokens + reasoning_reserve_tokens` | 配置后在 Extract 请求前估算完整 messages 预算；超预算时先裁剪非候选上下文 |
| `verifier_context_window_tokens` | 条件配置 | `null` | `null` | `null` 或大于 `verifier_max_output_tokens + reasoning_reserve_tokens` | 配置后在 Ground 请求前估算完整 messages 预算；Ground 无外围上下文可裁，超预算直接失败 |
| `reasoning_reserve_tokens` | 推荐可配置 | `0` | `0` | 大于等于 0 | 给可能产生的推理 token 预留预算，只参与请求前估算 |
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 约束 | 测试要求 |
| --- | --- | --- | --- | --- | --- |
| `database_path` | 部署必配 | 无（必填） | `data/ram-a-memory.sqlite` | 非空持久化 SQLite 文件路径；生产不接受 `:memory:`；不得与 `case_library.index_store` 相同；生产部署建议：RPM 使用绝对路径 `/var/lib/ram-a/ram-a-memory.sqlite` | 使用推荐绝对路径时服务启动成功、`/ready` 返回 2xx，并生成 SQLite/WAL 文件；空路径和 `:memory:` 必须启动失败；父目录只读或运行用户无写权限时必须启动失败并输出明确日志 |

`data/ram-a-memory.sqlite` 适合源码目录运行，不作为 RPM 生产部署建议。相对路径按进程工作目录解析；
...
| `embedding_model` | 部署必配 | 无（必填） | `hash` | 非空；外部 Provider 写实际模型名 |
| `embedding_dimensions` | 部署必配 | 无（必填） | `1024` | 大于 0；hash 自测 1024；外部 Provider 必须使用模型实际输出维度；已存向量与查询向量维度必须一致 |
| `extractor_model` | 部署必配 | 无（必填） | `GLM-5.2`（示例，按已验证模型替换） | 非空；使用已验证能稳定输出 JSON 的 Chat 模型；模型是否存在由联通测试验证 |
| `verifier_model` | 部署必配 | 无（必填） | `GLM-5.2`（示例，按已验证模型替换） | 非空；用于 Ground，也支持独立配置；推荐与 `extractor_model` 使用相同模型 |
...
| `reasoning_effort` | 条件配置 | `null` | `null` | 配置时非空；透传给 Chat 请求；与 `enable_thinking` 不能同时配置；GLM Coding Plan 已验证可用 `none` |
| `enable_thinking` | 条件配置 | `null` | `null` | 布尔值；透传给 Chat 请求；与 `reasoning_effort` 不能同时配置；仅在 Provider 明确支持时使用 `false` |
| `send_temperature` | 推荐可配置 | `true` | `true` | `true` 时发送 `temperature`，`false` 时省略；兼容服务不接受 temperature 时设 `false` |
| `temperature` | 推荐可配置 | 0.0 | 0.0 | 有限数，范围 -2.0..=2.0；记忆抽取推荐低温 |
| `output_token_parameter` | 固定值枚举 | `max_tokens` | `max_tokens` | 只接受 `max_tokens` 或 `max_completion_tokens`；单位是输出 token；按 Provider 协议选择 |
| `structured_output` | 固定值枚举 | `prompt_only` | `prompt_only` | 只接受 `prompt_only`、`json_object`、`json_schema`；未经 smoke test 时保持 `prompt_only` |
| `reasoning_only_retry` | 推荐可配置 | `false` | `false` | 只有 `reasoning_content` 而最终 `content` 为空时，至多纠正重试一次；生产部署建议：GLM Coding Plan 配置为 `true` |
| `json_repair_attempts` | 推荐可配置 | 0 | 0 | 只允许 0 或 1；只修复 Extract/Ground 返回的非合格 JSON；需要兼容弱 JSON 模型时设 1 |
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 约束与生效条件 |
| --- | --- | --- | --- | --- |
| `enabled` | 条件配置 | `false` | `false` | `true` 时要求 `retrieval.mode=hybrid` 并构建 Rerank 客户端 |
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 约束与生效条件 |
| --- | --- | --- | --- | --- |
| `rag_store` | 部署可配置 | `data/memory-cases.sqlite` | `data/memory-cases.sqlite` | 非空、非 `:memory:`，且不同于 `index_store`；生产部署建议：使用独立持久化文件 |
| `index_store` | 部署可配置 | `data/memory-cases-index.sqlite` | `data/memory-cases-index.sqlite` | 非空、非 `:memory:`，且不同于 `rag_store` 和个人记忆库；生产部署建议：使用独立持久化文件 |
| `source_dir` | 条件配置 | `null` | `crates/memory-cases/test/accuracy_docs`（示例，按环境替换） | 配置时路径非空；目录可读性由集成测试验证；仅需启动时自动扫描本地案例时填写 |
| `api_token_env` | 条件配置 | `null` | `RAM_A_CASES_ADMIN_TOKEN`（示例，按环境替换） | 配置后启用管理 API；环境变量名及值必须非空、无前后空格；不使用案例管理 REST API 时设 `null` |
| `ingestion_poll_ms` | 推荐可配置 | 1000 | 1000 | 大于 0；案例摄入 Worker 轮询间隔 |
| `embedding_provider` | 固定值枚举 | `openai_compatible` | `hash` | 枚举同 `providers.embedding_provider`；不要依赖默认 Provider 与默认模型的组合；生产部署建议：明确配置 `openai_compatible` |
| `embedding_api_key_env` | 条件配置 | `null` | `null` | `null` 回退到 `providers.api_key_env`；独立案例 Embedding 服务才填写 |
| `embedding_base_url` | 条件配置 | `null` | `null` | `null` 回退到 `providers.base_url`；独立案例 Embedding 服务才填写 |
| `embedding_model` | 推荐可配置 | `hash` | `hash` | 非空；生产写实际模型 |
| `embedding_dimensions` | 推荐可配置 | 1024 | 1024 | 大于 0；生产与实际模型一致 |
| `chunk_size` | 推荐可配置 | 160 | 160 | 大于 0；案例切块大小 |
| `summary_llm_model` | 条件配置 | `null` | `null` | 非 `null` 时启用摘要模型；不需要模型摘要时保持 `null` |
| `summary_llm_api_key_env` | 条件配置 | `null` | `null` | `null` 回退到 `providers.api_key_env`；摘要服务使用独立密钥时填写 |
| `summary_llm_base_url` | 条件配置 | `null` | `null` | `null` 回退到 `providers.base_url`；摘要服务使用独立端点时填写 |
| `summary_llm_timeout_ms` | 推荐可配置 | 30000 | 30000 | 大于 0；只在摘要模型启用时实际调用 |
| `default_library` | 部署必配 | 无（必填） | `ops`（示例，按环境替换） | 非空，必须引用 `libraries[].name`；选择最常用逻辑库 |
| `libraries` | 部署必配 | 无（必填） | 一项：`ops` / `openeuler-ops-cases` / `tenant-local`（示例，按环境替换） | 非空；`name` 唯一 |
| `libraries[].name` | 部署必配 | 无（必填） | `ops`（示例） | 非空、无前后空格、数组内唯一；使用稳定、面向调用方的逻辑库名 |
| `libraries[].dataset_id` | 部署必配 | 无（必填） | `openeuler-ops-cases`（示例） | 非空、无前后空格；使用内部稳定 dataset ID |
| `libraries[].tenant_ids` | 部署必配 | 无（必填） | `["tenant-local"]`（示例） | 非空数组，元素非空且无前后空格；明确列出允许访问的租户 |
...
| 字段 | 类别 | 未设置兜底值 | 交付配置默认值 | 约束与生效条件 |
| --- | --- | --- | --- | --- |
| `llm_api_key_env` | 部署必配 | 无（必填） | `GRAPH_LLM_API_KEY`（示例，按环境替换） | 非空；功能开启时环境变量必须存在 |
| `llm_base_url` | 部署可配置 | `https://openrouter.ai/api/v1` | `http://127.0.0.1:8000/v1`（本地端点，按环境替换） | URL 安全规则同主 Provider；生产部署建议：写实际 OpenAI-compatible 图模型端点 |
| `llm_model` | 部署必配 | 无（必填） | `GLM-5.2`（示例，按已验证模型替换） | 非空；使用支持图 schema 输出的模型 |
...
| `retrieval.fail_open` | 推荐可配置 | `false` | `false` | `true` 时图检索失败退回非图结果；不控制图摄入失败；生产部署建议：图是增强通道且要求核心检索可用时用 `true` |
...
| 环境变量 | 类别 | 未设置兜底值 | 固定值或约束 |
| --- | --- | --- | --- |
| `RAM_A_MEM_CONFIG` | 部署可配置 | 无 | 文件路径；优先级低于 CLI `--config`；未使用 `--config` 时指向交付配置 |
| `RAM_A_LOG_FORMAT` | 固定值枚举 | `json` | 只接受 `json`、`compact`，非法值启动失败；终端调试建议 `compact` |
| `RAM_A_LOG_SOURCE` | 固定值枚举 | `false` | 只接受 `true`、`false`，非法值启动失败；现场定位建议临时设 `true` |
| `RUST_LOG` | 部署可配置 | 服务默认过滤级别 | tracing filter 表达式；不改变业务结果 |
...
1. 从交付配置默认值生成一个字段变体，注入有效 Token 和 Provider 环境变量。
...
| 全部未设置兜底值 | `feature_http_and_provider_defaults_are_stable`、`graph_and_case_library_defaults_are_stable`、`http_limit_defaults_are_stable_and_supported`、`pipeline_defaults_and_boundaries_are_stable`、`retrieval_defaults_preserve_current_hybrid_behavior` |
