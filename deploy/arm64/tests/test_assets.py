...
class Arm64ImageAssetsTest(unittest.TestCase):
    ...
    def test_ram_a_config_uses_hash(self):
        ...
        self.assertEqual(1600, config["pipeline"]["extractor_max_output_tokens"])
        self.assertEqual(1000, config["pipeline"]["verifier_max_output_tokens"])
        self.assertIsNone(config["pipeline"]["extractor_context_window_tokens"])
        self.assertIsNone(config["pipeline"]["verifier_context_window_tokens"])
        self.assertEqual(0, config["pipeline"]["reasoning_reserve_tokens"])
        self.assertEqual("none", config["providers"]["reasoning_effort"])
        self.assertIsNone(config["providers"]["enable_thinking"])
        self.assertTrue(config["providers"]["send_temperature"])
        self.assertEqual(0, config["providers"]["temperature"])
        self.assertEqual("max_tokens", config["providers"]["output_token_parameter"])
        self.assertEqual("prompt_only", config["providers"]["structured_output"])
        self.assertTrue(config["providers"]["reasoning_only_retry"])
        self.assertEqual(1, config["providers"]["json_repair_attempts"])
