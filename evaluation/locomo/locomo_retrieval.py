...
def main():
    ...
    parser.add_argument("--input-format", choices=("memory-bench", "mem0"), default="memory-bench")
...
def evaluate_retrieval(dataset, search_results, input_path):
    rows = []
    by_category = defaultdict(list)
    for item in search_results:
        row = evaluate_query(dataset, item)
        if row is None:
            continue
        rows.append(row)
...
def render_failure_table(items):
    ...
    for item in items:
        rows.append(
            ...
            f'{render_text_cell(", ".join(item.get("retrieved_evidence", [])[:10]))}'
